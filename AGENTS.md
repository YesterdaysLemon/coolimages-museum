...
- Test URLs: `?test` enables drag-to-look and skips pointer lock; `&mute` starts silent.
  `window.museum` exposes `start()`, `teleport(room)`, `look(x, z, yaw, pitch, y?)`, `inspect()`, `back()`, `walkable(x, z, y)`, `plan` (forms, doors, stairs) and `portals` (the door graph with each door's height; `arrives` is false if a door has no way out on the other side).
