...
- Test URLs: `?test` enables drag-to-look and skips pointer lock; `&mute` starts silent.
  `window.museum` exposes `start()`, `teleport(room)`, `look(x, z, yaw, pitch)`, `inspect()`, `back()` and `portals` (the door graph; `arrives` is false if a door has no way out on the other side).
